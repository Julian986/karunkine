...
export const WANDA_CUSTOMER_SESSION_COOKIE = "wanda_customer_session";
...
function sessionSecret(): string {
  ...
  return s || "dev-wanda-customer-session-cambiar";
}

/** Cookie firmada (HttpOnly) con el teléfono normalizado solo dígitos. */
export function mintCustomerSessionToken(phoneDigits: string, maxAgeMs = 60 * 24 * 60 * 60 * 1000): string {
...
export function readCustomerSessionPhoneDigits(token: string | undefined): string | null {
