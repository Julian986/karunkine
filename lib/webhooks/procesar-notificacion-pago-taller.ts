...
/**
 * Confirma inscripción al taller si MP devuelve pago approved y monto coincide.
 */
export async function procesarNotificacionPagoTallerInscripcion(
  ...
  if (status !== "approved") {
    ...
    await registrarEventoWebhook(db, {
      ...
      outcome: "ignored",
      detail: `inscripción taller: pago no aprobado: ${pago.status}`,
      turnoId: extRef,
      paymentId: pid,
    });
    return {
  ...
  if (currency !== "ARS") {
    await registrarEventoWebhook(db, {
      ...
      payloadSummary: audit.payloadSummary,
      outcome: "error",
      detail: `inscripción taller: currency_id ${currency}`,
      turnoId: extRef,
      paymentId: pid,
    });
  ...
  const inscripcion = await db.collection("taller_inscripciones").findOne({ _id: inscripcionOid });
  if (!inscripcion) {
    await registrarEventoWebhook(db, {
      receivedAt: new Date(),
      source: audit.source,
      ...
      payloadSummary: audit.payloadSummary,
      outcome: "error",
      detail: "inscripción taller no encontrada",
      turnoId: extRef,
      paymentId: pid,
    });
  ...
  if (!Number.isFinite(montoMp) || Math.round(montoMp) !== Math.round(precioEsperado)) {
    await registrarEventoWebhook(db, {
      ...
      payloadSummary: audit.payloadSummary,
      outcome: "error",
      detail: `inscripción taller: monto MP ${montoMp} !== ${precioEsperado}`,
      turnoId: extRef,
      paymentId: pid,
    });
  ...
  const now = new Date();
  const updateResult = await db.collection("taller_inscripciones").updateOne(
    {
      _id: inscripcionOid,
  ...
  if (updateResult.matchedCount === 0) {
    const actual = await db.collection("taller_inscripciones").findOne(
      { _id: inscripcionOid },
      { projection: { estado: 1 } },
    );
    if (actual?.estado === "confirmado") {
      ...
      await registrarEventoWebhook(db, {
        ...
        payloadSummary: audit.payloadSummary,
        outcome: "duplicate",
        detail: "inscripción taller ya confirmada",
        turnoId: extRef,
        paymentId: pid,
      });
      return {
    ...
    await registrarEventoWebhook(db, {
      ...
      payloadSummary: audit.payloadSummary,
      outcome: "error",
      detail: "inscripción taller no actualizable",
      turnoId: extRef,
      paymentId: pid,
    });
    return {
  ...
  await registrarEventoWebhook(db, {
    ...
    payloadSummary: audit.payloadSummary,
    outcome: "processed",
    detail: "inscripción taller confirmada",
    turnoId: extRef,
    paymentId: pid,
  });
